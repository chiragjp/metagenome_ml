...
def remove_short(inputName,outputNameFilt,outputNameRem,diamondIndex):
	import sys
	import numpy as np
	import os
	names=[]
	...
	os.system("diamond blastx --query 'removed_sequences_temp' --id '95' -d '%s' -k '1' -o 'removed_sequences_temp_aligned'"%(diamond,diamondIndex))
...
#arguments: input gene catalog, output gene filtered gene catalog, list of removed sequences, diamond index to align short sequences against

remove_short(sys.argv[1],sys.argv[2],sys,argv[3],sys.argv[4])