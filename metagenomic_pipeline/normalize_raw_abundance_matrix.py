...
###NOTE you may need to remove the last line of the raw count matrix before running this script, as samtools sometimes outputs an erroneous gene (called "*") 
...
def calc_abundance(sublist):
	gene = sublist[0]
	gene_length = int(sublist[1])
	values = sublist[2:]
	abundances = map(lambda x: x/gene_length, map(lambda y: int(y),values))
...
by_row = len(normalized_by_length)
